var getSpecialities = function () {
  $.post("/data/CollegeSpeciality", function (data) {
    ...
    $("#addSpeciality").html("");
    $("#addSpeciality").append(
      '<option style="display:none" selected>空</option>'
    );
    $.each(data, function (index, item) {
      var typeStr = '<optgroup label="' + item["ColName"] + '">';
      for (let speName of item["SpeName"]) {
        typeStr +=
          '<option value="' + speName[1] + '">' + speName[0] + "</option>";
      }
      typeStr += "</optgroup>";
...
var getClassrooms = function () {
  $.post("/data/ClassroomInfo", function (data) {
    ...
    $.each(data, function (index, item) {
      var typeStr =
        '<option value="' + item["No"] + '">' + item["position"] + "</option>";
...
var check_addStudent = function () {
  if (
    ...
    document.getElementById("addSpeciality").value != "空" &&
...
var check_addTeacher = function () {
  if (
    document.getElementById("addName").value &&
    document.getElementById("addBirthday").value &&
    document.getElementById("addCollege").value != "空" &&
    document.getElementById("addGender").value
  ) {
    return true;
  } else {
...
var check_addExam = function () {
  if (
    document.getElementById("addCourseNo").value != "空" &&
    document.getElementById("addDay").value &&
    document.getElementById("addBeginTime").value &&
    document.getElementById("addEndTime").value &&
    document.getElementById("addTeacherNo").value != "空" &&
    document.getElementById("addClassroomNo").value != "空"
  ) {
    return true;
  } else {
    alert("请填写完整！");
