...
var getTeachCourses = function () {
  $.post("/data/TeachCourseInfo", function (data) {
    ...
    $("#addCourseNo").html("");
    $("#addCourseNo").append(
      '<option style="display:none" value="" selected>空</option>'
    );
    $.each(data["rows"], function (index, item) {
      var typeStr =
        '<option value="' +
...
var check_addEvaluation = function () {
  if (
    document.getElementById("addCourseNo2").value != "空" &&
    document.getElementById("addEva").value
  ) {
    return true;
  } else {
    alert("请填写完整！");
...
var addTeachApplication_submit = function () {
  if (!check_addTeachApplication()) return;
  var data = {};
  data["choice"] = "submit";
...
var addStartApplication_submit = function () {
  if (!check_addStartApplication()) return;
  var data = {};
  data["choice"] = "submit";
